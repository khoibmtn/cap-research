...
export default function CrosstabTab({ patients }: { patients: Patient[] }) {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Add/Remove variable pool */}
            <div className="flex flex-wrap items-center gap-2">
                <span className="text-xs text-gray-500">Biến đang chọn ({selectableVars.length}):</span>
                {selectableVars.map(v => {
                    const isDefault = DEFAULT_VARIABLES.some(dv => dv.id === v.id);
                    return (
                        <span key={v.id} className="inline-flex items-center gap-1 px-2 py-0.5 bg-gray-100 border border-gray-200 rounded-full text-xs text-gray-700">
                            {v.label}
                            {!isDefault && (
                                <button
                                    onClick={() => {
                ...
                {addableVars.length > 0 && (
                    <div className="relative">
                        <button
                            ...
                            className="inline-flex items-center gap-1 px-2 py-0.5 bg-primary-50 border border-primary-200 rounded-full text-xs text-primary-700 hover:bg-primary-100"
                        >
                            <Plus className="w-3 h-3" /> Thêm biến
                        </button>
                        {showAddMenu && (
                            <div className="absolute top-full left-0 mt-1 w-56 bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-64 overflow-auto">
                                {Object.entries(groupVars(addableVars)).map(([group, vars]) => (
                                    <div key={group}>
                                        <div className="px-3 py-1 text-[10px] font-semibold text-gray-400 uppercase bg-gray-50">{group}</div>
                                        {vars.map(v => (
                                            <button
                                                key={v.id}
                                                onClick={() => {
                                                    setActiveVarIds(prev => [...prev, v.id]);
                                                    if (addableVars.length <= 1) setShowAddMenu(false);
                                                }}
                                                className="w-full text-left px-3 py-1.5 text-xs hover:bg-primary-50 text-gray-700"
                                            >
                                                {v.label}
                                            </button>
                                        ))}
